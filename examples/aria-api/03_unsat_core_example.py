...
def basic_core():
    ...
    if r == z3.unsat:
        s2 = z3.Solver()
        a = [z3.Bool(f"c{i}") for i in range(len(cs))]
...
def named_core():
    ...
    s.add(
        z3.Implies(c1, a),
        z3.Implies(c2, z3.Not(a)),
        z3.Implies(c3, z3.Or(a, b)),
        z3.Implies(c4, z3.Not(b)),
        z3.Implies(c5, c),
    )
    r = s.check([c1, c2, c3, c4, c5])
    print(f"Named: {r}")
    if r == z3.unsat:
