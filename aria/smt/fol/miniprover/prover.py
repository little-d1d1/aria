...
def unify(term_a, term_b):
    ...
    if (isinstance(term_a, Function) and isinstance(term_b, Function)) or (
        ...
        if len(term_a.terms) != len(term_b.terms):
            return None
        substitution = {}
        for i, a_term in enumerate(term_a.terms):
            a = a_term
            b = term_b.terms[i]
...
class Sequent:
    """Represents a sequent in the proof system."""
    ...
    def freeUnificationTerms(self):  # pylint: disable=invalid-name
        """Get all free unification terms in the sequent."""
        result = set()
        for formula in self.left:
    ...
    def getVariableName(self, prefix):  # pylint: disable=invalid-name
        """Get a fresh variable name with the given prefix."""
    ...
    def getUnifiablePairs(self):  # pylint: disable=invalid-name
        """Get all pairs of unifiable formulas from left and right."""
        pairs = []
...
def proveFormula(axioms, formula):  # pylint: disable=invalid-name
    """
    Prove a formula from a set of axioms.
