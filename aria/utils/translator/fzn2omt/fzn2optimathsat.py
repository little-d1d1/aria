...
try:
    from ....translator.fzn2omt import common
except ImportError:
    import aria.utils.translator.fzn2omt.common as common
...
def binary_filename():
    ...
    return "optimathsat.exe" if os.name == "nt" else "optimathsat"
...
def optimathsat_solve(config, solver_config=None):
    ...
    subprocess.run(args, universal_newlines=True, check=True)
...
def optimathsat_compile(config, solver_config=None):
    ...
    result = subprocess.run(
        args, capture_output=True, universal_newlines=True, check=True
    )
    ...
    if config.ovars and common.is_file_empty(config.ovars):
        common.eprint(
            (
                "error: failed to generate mapping between "
                ...
                if is_opt_problem:
                    priority_cfg = list(
                        filter(
                            ...
                            map(
                                lambda g: g[14:],
                                filter(lambda f: "-opt.priority=" in f, solver_config),
                            ),
                        )
                    )
                    if priority_cfg:
...
def optimathsat_parse_cmdline_options():
    ...
    parser = argparse.ArgumentParser(
        description=("A tool for solving FlatZinc " "problems with OptiMathSAT."),
    ...
    main_group.add_argument(
        ...
        metavar="<filename>",
        type=argparse.FileType("w"),
        action=common.check_file_ext(),
        help=(
            "Filename for the generated mapping between "
    ...
    main_group.add_argument(
        "--compile-raw",
        help=(
            "Generate a raw SMT-LIB formula instead of one " "suitable for OptiMathSAT."
        ),
        action="store_true",
        default=False,
    )
    ...
    search_group.add_argument(
        "--partial-solutions",
        help=("Print any sub-optimal solution satisfying " "the input model."),
        action="store_true",
        default=False,
    )
    ...
    search_group.add_argument(
        "--all-solutions-opt",
        help=(
            "Print all solutions of the input problem. "
    ...
    ignore_group.add_argument(
        "--num-threads",
        "-p",
        help=("Number of threads. " "(OptiMathSAT is single threaded)"),
        metavar="N",
        type=int,
        default=1,
    )
