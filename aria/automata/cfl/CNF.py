...
def loadModel(
    ...
    p_var = file_content.split("Productions:\n")[1]

    return cleanAlphabet(k_var), cleanAlphabet(v_var), cleanProduction(p_var)
...
def seekAndDestroy(
    target: str, productions: List[Tuple[str, List[str]]]
...
def setupDict(
    productions: List[Tuple[str, List[str]]], variables: List[str], terms: List[str]
) -> Dict[str, str]:
    result: Dict[str, str] = {}
    for production in productions:
        #
        if (
            production[left] in variables
            and production[right][0] in terms
            and len(production[right]) == 1
        ):
...
# Add S0->S rule––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––START
def START(
    productions: List[Tuple[str, List[str]]], variables: List[str]
) -> List[Tuple[str, List[str]]]:
    variables.append("S0")
...
def unit_routine(
    rules: List[Tuple[str, List[str]]], variables: List[str]
) -> List[Tuple[str, List[str]]]:
...
if __name__ == "__main__":
    ...
    Productions = STBDU_transformation(model_path)
    print(prettyForm(Productions))
    print(len(Productions))
    with open("out.txt", "w", encoding="utf-8") as out_file:
        out_file.write(prettyForm(Productions))
...
def start_transform(
    start_symble: str, grammar: Dict[str, List[List[str]]]
